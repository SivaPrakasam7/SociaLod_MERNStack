...
const TestCase = {
  ...
  async Tokenerfunc() {
    ...
    enc = (
      await tokener.generate(
        { _id: "60f5046082e8f4290dba8fbf", name: "siva" },
        key
      )
    ).message.token;
    ...
    console.table({
      Invalid_Token_Verification: await tokener.verify(enc, key),
    });
  ...
  async Single() {
    await xsengine.scrap("sivaprakasam7", require("../src/entities/github"));
    console.log(JSON.stringify(await xsengine.filter(), null, 4));
    xsengine.clear();
    await xsengine.scrap("harishwarrior", require("../src/entities/github"));
    console.log(JSON.stringify(await xsengine.filter(), null, 4));
  },
  async Test() {
