...
module.exports = {
    query: null, url: null, edata: null, data: { Query: [], Link: [], Screenshot: [], performance: [], Error: [] }, options: new chrome.Options(), dirver: 0,
        ...
        if (edata && query) {
            ...
            for (const [k, v] of Object.entries(this.edata)) {
                if (k == "Options") {
                    ...
                    if (await this.screenshot(this.url).catch(err => { return true })) {
