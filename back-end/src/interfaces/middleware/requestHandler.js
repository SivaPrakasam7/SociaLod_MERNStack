...
exports.register = async (req, res, next) => {
  const info = await authendication.register(
    req.status.uid.message.email,
    req.body
  );
...
exports.resetpass = async (req, res, next) => {
  const info = await authendication.reset(
    req.status.uid.message,
    req.body.password
  );
  if (!info.err) return res.status(201).send(info);
  else return next(info);
...
exports.connect = async (req, res, next) => {
  const info = await useraccess.connect(req.status.uid.message.id, req.body);
  if (!info.err) return res.status(201).send(info);
  else return next(info);
};
...
exports.report = async (req, res, next) => {
  const info = await useraccess.report(req.status.uid.message.id, req.body);
  if (!info.err) return res.status(201).send(info);
  else return next(info);
};
...
exports.mailer = async (req, res, next) => {
  const info = await useraccess.mailService(
    req.status.uid.message.email,
    req.body
  );
  if (!info.err) return res.send(info);
