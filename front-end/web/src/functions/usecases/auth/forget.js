...
export default async function Forgetsubmit(token) {
    ...
    if (form.fpassword.value === form.fcpassword.value) {
        const info = await fetch(`${window.env.BURL}/api/auth/reset`, { method: "POST", headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ password: form.fpassword.value }) })
