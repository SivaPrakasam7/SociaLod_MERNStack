...
export default async function Add() {
    ...
    const info = await fetch('http://192.168.43.175:5000/api/service/new_media', { method: "POST", headers: { 'Authorization': `Bearer ${document.cookie.replace("CID=", "")}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ username: scrap.slice(1).join('/'), site: scrap[0] }) })
