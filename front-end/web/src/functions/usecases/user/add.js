...
export default async function Add() {
    ...
    if (/(https?:\/\/)?[a-z.-]+\.[a-z]{2,6}[/\w.-]*\/?/g.test(dt.value)) {
        ...
        const info = await fetch(`${window.env.BURL}/api/service/new_media`, { method: "POST", headers: { 'Authorization': `Bearer ${document.cookie.replace("CID=", "")}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ username: scrap.slice(1).join('/'), site: scrap[0] }) })
